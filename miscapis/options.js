...
function loadGeneralSettings() {
	var ids = ["linksfoundindicator", "showpopups", "popupduration", "hearpopups", "catchfromcontextmenu", "catchfrompage", "linkmatches", "catchfromnewtab", "registerDelay"];
	for (const id of ids) {
		getSetting(document.getElementById(id));
	}
...
function setSetting(e, val) {
	chrome.storage.local.set({ [e.id]: (val == undefined) ? "" : val });
}

function getSetting(e) {
	if (!e) return;
	chrome.storage.local.get([e.id], (result) => {
...
function loadMatches() {
	chrome.storage.local.get(["linkmatches"], (result) => {
